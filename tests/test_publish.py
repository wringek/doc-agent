...
def test_write_doc_basic(tmp_path, monkeypatch):
    ...
    # Mock the docs directory to use a temporary path
    ...
    doc_path = tmp_path / "docs" / "test_function.md"
...
def test_write_doc_all_sections(tmp_path, monkeypatch):
    ...
    # Check content
    content = (tmp_path / "docs" / "complete_function.md").read_text()
...
def test_write_doc_empty_sections(tmp_path, monkeypatch):
    ...
    content = (tmp_path / "docs" / "minimal_function.md").read_text()
    
    # Check that empty sections are not included
    assert "## Purpose" not in content
    assert "## Examples" not in content

def test_write_doc_creates_docs_dir(tmp_path, monkeypatch):
